...
function virusCheckRecursive(str, viruses) {
  ...
  if (str.length === 0 || !viruses) {
    ...
    const thereIsViruses = viruses
      .split("|")
      .filter((e) => e.toLowerCase() === str[0].toLowerCase())
    return (
      (thereIsViruses.length > 0 ? 1 : 0) +
      virusCheckRecursive(str.substr(1), viruses)
    )
  }
}
