...
function travelingIndonesia(arr, emoney) {
  ...
  arr.map((el) => {
    ...
    const calculateRoute = Math.abs(iDepature - iDestination)
    // HITUNG JUMLAH YANG HARUS DI  BAYAR
    res.push({
      name: temp[0],
      departureCity: temp[1],
      destinationCity: temp[2],
      transport: temp[3],
      totalCost:
        transportation[temp[3]] * calculateRoute -
        (payment[emoney] / 100) * (transportation[temp[3]] * calculateRoute),
    })
  })
  return res
}
